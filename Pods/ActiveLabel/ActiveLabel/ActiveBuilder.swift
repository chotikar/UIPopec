...
struct ActiveBuilder {
    ...
    private static func createElementsIgnoringFirstCharacter(from text: String,
        ...
        for match in matches where match.range.length > 5 {
            let range = NSRange(location: match.range.location + 4, length: match.range.length - 4)
            var word = nsstring.substring(with: range)
            if word.hasPrefix("#Fac") {
                word.remove(at: word.startIndex)
            }
            else if word.hasPrefix("#Pro") {
                word.remove(at: word.startIndex)
            }
