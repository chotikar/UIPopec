...
@IBDesignable open class ActiveLabel: UILabel {
    ...
    open var enabledTypes: [ActiveType] = [.hashtagFac, .hashtagPro, .url]
    
    open var urlMaximumLength: Int?
    
    open var configureLinkAttribute: ConfigureLinkAttribute?
    
    @IBInspectable open var hashtagFacColor: UIColor = .blue {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var hashtagFacSelectedColor: UIColor? {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var hashtagProColor: UIColor = .blue {
        didSet { updateTextStorage(parseText: false) }
    }
    @IBInspectable open var hashtagProSelectedColor: UIColor? {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    // MARK: - public methods
    open func handleHashtagFacTap(_ handler: @escaping (String) -> ()) {
        hashtagFacTapHandler = handler
    }
    
    open func handleHashtagProTap(_ handler: @escaping (String) -> ()) {
        hashtagProTapHandler = handler
    }
    ...
    open func removeHandle(for type: ActiveType) {
        switch type {
        case .hashtagFac:
            hashtagFacTapHandler = nil
        case .hashtagPro:
            hashtagProTapHandler = nil
        case .url:
            urlTapHandler = nil
        }
    }
    
    open func filterHashtagFac(_ predicate: @escaping (String) -> Bool) {
        hashtagFacFilterPredicate = predicate
        updateTextStorage()
    }
    
    open func filterHashtagPro(_ predicate: @escaping (String) -> Bool) {
        hashtagProFilterPredicate = predicate
        updateTextStorage()
    }
    ...
    func onTouch(_ touch: UITouch) -> Bool {
        ...
        case .ended:
            ...
            case .hashtagFac(let hashtagFac): didTapHashtagFac(hashtagFac)
            case .hashtagPro(let hashtagPro): didTapHashtagPro(hashtagPro)
    ...
    internal var hashtagFacTapHandler: ((String) -> ())?
    internal var hashtagProTapHandler: ((String) -> ())?
    internal var urlTapHandler: ((URL) -> ())?
    
    fileprivate var hashtagFacFilterPredicate: ((String) -> Bool)?
    fileprivate var hashtagProFilterPredicate: ((String) -> Bool)?
    ...
    fileprivate func addLinkAttribute(_ mutAttrString: NSMutableAttributedString) {
        ...
        attributes[NSForegroundColorAttributeName] = hashtagFacColor
        
        for (type, elements) in activeElements {
            
            switch type {
            case .hashtagFac: attributes[NSForegroundColorAttributeName] = hashtagFacColor
            case .hashtagPro: attributes[NSForegroundColorAttributeName] = hashtagProColor
    ...
    fileprivate func parseTextAndExtractActiveElements(_ attrString: NSAttributedString) -> String {
        ...
        for type in enabledTypes where type != .url {
            ...
            if type == .hashtagFac {
                filter = hashtagFacFilterPredicate
            } else if type == .hashtagPro {
                filter = hashtagProFilterPredicate
            }
    ...
    fileprivate func updateAttributesWhenSelected(_ isSelected: Bool) {
        ...
        if isSelected {
            ...
            switch type {
            case .hashtagFac: selectedColor = hashtagFacSelectedColor ?? hashtagFacColor
            case .hashtagPro: selectedColor = hashtagProSelectedColor ?? hashtagProColor
            ...
            switch type {
            case .hashtagFac: unselectedColor = hashtagFacColor
            case .hashtagPro: unselectedColor = hashtagProColor
    ...
    fileprivate func didTapHashtagFac(_ hashtag: String) {
        guard let hashtagFacHandler = hashtagFacTapHandler else {
            delegate?.didSelect(hashtag, type: .hashtagFac)
            return
        }
       hashtagFacHandler(hashtag)
    }
    
    fileprivate func didTapHashtagPro(_ hashtag: String) {
        guard let hashtagProHandler = hashtagProTapHandler else {
            delegate?.didSelect(hashtag, type: .hashtagPro)
            return
        }
        hashtagProHandler(hashtag)
    }
